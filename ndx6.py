...
##    s  = mpf.make_mpf_style(base_mpf_style='yahoo',mavcolors=['#ef5714','#e7d111','#9f4878','#ef5714','#ef5714','#ff00dd','#1900ff'])
    ...
    print(df,' hhh',df.columns)

    
    df33=yf.Ticker(g).history(period = '4d', interval = '1d')
    print(g,'    ',df33.shape)
    cto=df33['Close'][-1]
    c1day=df33['Close'][-2]
...
##        sys.exit()
        ...
        mpf.plot(df, block=False,type='candle', addplot=adp,volume=True, figratio=(15, 15),
                 mav=(200,100,50,20,10,5,3),title=ticker.upper(),
...
##                    ' MA3=Black'                                       ,
                 ...
                 hlines=dict(hlines=[cto,c1day],colors=['g','r'],linestyle='-.')
##                 ,vlines=dict(vlines=zz
##                              , colors=['g', 'g', 'r', 'r'],linestyle='dotted'
                 ,vlines=dict(vlines=mt
                              , colors=mc,linestyle='dotted'
          


                              )
##                 ,vlines=dict(vlines=mt3,linestyle='-.',colors=['b']
