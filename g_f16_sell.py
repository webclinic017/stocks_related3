def inputs2(df,z,buy_price_gg):
    import f16_condt
    from f16_condt import con_sell_up
    sell_status='no'
    
##    sell_condition=(df['ADL'].loc[z] < 0)
    sell_condition=con_sell_up(df,z,xs)

##    or (df['Close'].loc[z] - df['Close'].shift(1).loc[z] < .1)\
##                    or (df['Close'].loc[z]-buy_price_gg < 0.5)
...
def f16_sellp(xs,b6,df,buy_price_gg,buy_status):
    import sys
    sell_status='no'
...
##        f = open('/home/az2/b_94519.txt', 'a+')
        sell_condition,sell_status=inputs2(df,z,buy_price_gg)
##        f.write(str('_')+sell_condition+str('_'))
##        f.write('\n')         
##        f.close()            
...
##    print(sell_price_gg,'-------------------------- sell_price_gg')
##    print(buy_price_gg,'-------------------------- buy_price_gg')
