...
########################################################################
    ...
    for x in df.index:
        ...
        df['MA->'].loc[x]='MA->'
        df['CCI/RSI->']='CCI/RSI->'
        df['swng'].loc[x]=df['High'].loc[x]-(df['Low'].loc[x])*.99999
...
##        df['Volumea']=(df['Volume']-df['Volume'].shift(1))
        ...
        df['swng_Low'].loc[x]=df['Low'].loc[x]-df['Low'].shift(1).loc[x]
        df['swng_High'].loc[x]=df['High'].loc[x]-df['High'].shift(1).loc[x]
        df['swng_open'].loc[x]=df['Open'].loc[x]-df['Open'].shift(1).loc[x]
        df['candle_color'].loc[x]=df['Close'].loc[x]-df['Open'].loc[x]
...
##                  df['MA->']
                  ...
                  df['swng_Low'], df['swng_High'], df['swng_open'], df['candle_color'] 
            ...
            'ppx','*','r1x','r2x','r3x','*','s1x','s2x','s3x','*','*',
             ...
             'Price_from_BBUP','Price_from_BBLWR','Price_from_BBMid','bol_wd',
              '3_5_crossovr','5_10_crossovr','10_21_crossovr','21_50_crossovr','50_100_crossovr','Price_3_crossovr'
                      ,'upward_pressure','downward_pressure','Closev','5_10_crossovr','f',    'swng_Low',
            'swng_High',
            'swng_open',
            'candle_color'
        ...
        'CDLDOJI','CDLDOJISTAR', 'CDLDRAGONFLYDOJI', 'CDLEVENINGDOJISTAR','CDLGRAVESTONEDOJI', 'CDLHAMMER', 'CDLXSIDEGAP3METHODS','*','*','CCI','DX', 'WILLR', 'RSI', 'adx', 'WilliamsR', 'ATR', 'ULTOSC', 'ROC','PLUS_DI', 'PLUS_DM', 'HT_DCPERIOD', 'HT_DCPHASE', 'sine',
         ...
         'Price_from_BBUP','Price_from_BBLWR','Price_from_BBMid','bol_wd',
          '3_5_crossovr','5_10_crossovr','10_21_crossovr','21_50_crossovr','50_100_crossovr','Price_3_crossovr','100_200_crossovr'
           ,'upward_pressure','downward_pressure','Closev','5_10_crossovr',    'swng_Low',
        'swng_High',
        'swng_open',
        'candle_color'
    ...
    for x in gg33b.index:  
        if gg33b['5_10_crossovr'].loc[x] > 0:
            if gg33b['5_10_crossovr'].shift(1).loc[x] < 0:
                p8.append(x)
    print('5_10_crossovr ---> ',p8,'  Buy')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'5-10-crossover-signal']='buy_5_10'
##    print(gg33b)
    a5=len(p8)            
...
####################### 5-10 crossover ###########################3
    ...
    for x in gg33b.index:  
        if gg33b['5_10_crossovr'].loc[x] < 0:
            if gg33b['5_10_crossovr'].shift(1).loc[x] > 0:
                p8.append(x)
    print('5_10_crossovr ---> ',p8,'  Sell')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'5-10-crossover-signal']='Sell_5_10'
##    print(gg33b)

    a6=len(p8)
    a7=a5 - a6
    if a7> 0:
        m2a='5-10 crossover-signal ----> Buy'
    if a7 < 0:
        m2a='5-10 crossover-signal ----> Sell'           

##################################################################   
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#######################  10-21 crossover ###########################3
    gg33b['10-21-crossover-signal']=''
    p8=[]
    ...
    for x in gg33b.index:  
        if gg33b['10_21_crossovr'].loc[x] > 0:
            if gg33b['10_21_crossovr'].shift(1).loc[x] < 0:
                p8.append(x)
    print('10_21_crossovr --> ',p8,'  Buy')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'10-21-crossover-signal']='buy_10_21'
##    print(gg33b)
    a5=len(p8)           
...
####################### 10-21 crossover ###########################3
    ...
    for x in gg33b.index:  
        if gg33b['10_21_crossovr'].loc[x] < 0:
            if gg33b['10_21_crossovr'].shift(1).loc[x] > 0:
                p8.append(x)
    print('10_21_crossovr --> ',p8,'  Sell')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'10-21-crossover-signal']='Sell_10_21'
##    print(gg33b)
    a6=len(p8)
    a7=a5 - a6
    if a7> 0:
        m2='10-21 crossover-signal ----> Buy'
    if a7 < 0:
        m2='10-21 crossover-signal ----> Sell'
                
##################################################################   
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#######################  '21_50_crossovr' ###########################3
    gg33b['21-50-crossover-signal']=''
    p8=[]
    ...
    for x in gg33b.index:  
        if gg33b['21_50_crossovr'].loc[x] > 0:
            if gg33b['21_50_crossovr'].shift(1).loc[x] < 0:
                p8.append(x)
    print('21_50_crossovr --> ',p8,'  Buy')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'21-50-crossover-signal']='buy_21_50'
##    print(gg33b)
    a5=len(p8)           
...
####################### 21-50 crossover ###########################3
    ...
    for x in gg33b.index:  
        if gg33b['21_50_crossovr'].loc[x] < 0:
            if gg33b['21_50_crossovr'].shift(1).loc[x] > 0:
                p8.append(x)
    print('21_50_crossovr --> ',p8,'  Sell')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'21-50-crossover-signal']='Sell_21_50'
##    print(gg33b)
    a6=len(p8)
    a7=a5 - a6
    if a7> 0:
        m3='21-50-crossover-signal ----> Buy'
    if a7 < 0:
        m3='21-50-crossover-signal ----> Sell'
    ...
    for x in gg33b.index:  
        if gg33b['50_100_crossovr'].loc[x] > 0:
            if gg33b['50_100_crossovr'].shift(1).loc[x] < 0:
                p8.append(x)
    print('50_100_crossovr --> ',p8,'  Buy')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'50-100-crossover-signal']='buy_50_100'
##    print(gg33b)
    a5=len(p8)  
...
####################### '50_100_crossovr' ###########################3
    ...
    for x in gg33b.index:  
        if gg33b['50_100_crossovr'].loc[x] < 0:
            if gg33b['50_100_crossovr'].shift(1).loc[x] > 0:
                p8.append(x)
    print('500_100_crossovr --> ',p8,' Sell')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'50-100-crossover-signal']='Sell_50_100'
##    print(gg33b)

    a6=len(p8)
    a7=a5 - a6
    if a7> 0:
        m4='50_100_crossovr ----> Buy'
    if a7 < 0:
        m4='50_100_crossovr ----> Sell'   

##################################################################
    ## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#######################  '100_200_crossovr' ###########################3
    gg33b['100-200-crossover-signal']=''
    p8=[]
    ...
    for x in gg33b.index:  
        if gg33b['100_200_crossovr'].loc[x] > 0:
            if gg33b['100_200_crossovr'].shift(1).loc[x] < 0:
                p8.append(x)
    print('100_200_crossovr --> ',p8,'  Buy')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'100-200-crossover-signal']='buy_100_200'
##    print(gg33b)
    a5=len(p8)           
...
####################### '100_200_crossovr' ###########################3
    ...
    for x in gg33b.index:  
        if gg33b['100_200_crossovr'].loc[x] < 0:
            if gg33b['100_200_crossovr'].shift(1).loc[x] > 0:
                p8.append(x)
    print('100_200_crossovr --> ',p8,' Sell')
    for x in gg33b.index:
        for y in p8:
            if x==y:
                gg33b.loc[x,'100-200-crossover-signal']='Sell_100_200'
    a6=len(p8)
    a7=a5 - a6
    if a7> 0:
        m5='100_200_crossovr ----> Buy'
    if a7 < 0:
        m5='100_200_crossovr ----> Sell'
    ...
    print('-------------------------------------------------------------------------------------------------------------------')
    print('ticker ', ticker)
    print('bolinger_mean= ',gg33b['bol_wd'].mean(),
          'bolinger_min=',gg33b['bol_wd'].min(),
          'bolinger_max=',gg33b['bol_wd'].max(),
          '***********','\n',
          'bolinger_now=',gg33b['bol_wd'].tail(1))


    print('\n\n')
    print('ticker ', ticker)
    print('100_200_crossovr  ',m5)
    print('50_100_crossovr   ',m4)
    print('21_50_crossovr    ',m3)
    print('10_21_crossovr    ',m2)
    print('5_10_crossovr     ',m2a)
    print('-------------------------------------------------------------------------------------------------------------------')
