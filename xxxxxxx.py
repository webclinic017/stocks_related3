...
############################################################    
def xz(periodb,intervalb,p,d2,d3,d4,d5):
    import time
    from time import time
...
##    df = yf.download('tsla', period='2d', interval='1m',prepost = True)
    df=yf.Ticker(ticker).history(period = '5d', interval = '1d',prepost = True, proxies=proxyb)
...
##        print(proxyb,'  ','\n\n',df)
    ...
    df['Vol_delta']=''
    df['Vol_deltan']=''
##    print(ticker,'   ',df[['Close','Volume']].tail(2))    
    ...
    for x in df.index:
        ...
        d2.append(df['ticker'].loc[x])
        d5.append(df['Vol_delta'].loc[x])
        d3.append(df['Close'].loc[x])
        d4.append(df['delta_price'].loc[x])


##    dp=pd.DataFrame([d2,d3,d5,df['Close'],d4,df.index])
    dp=pd.DataFrame([d2,d5,d3,d4])    
    dm=dp.T

    dm=dm.tail(1)
##    print(dm)

    print(dm.to_string(index=False,header=False))
...
sv=['spy']
sh=['arkk','spy']
sh=['arkk','^ndx','^gspc','^dji','spy','tna','qqq','tsla','^dji']
