...
##    alln=['aapl','t']
    ...
    for x in all:
        ...
        df['MA2'] = ta.EMA(df['fastk'], timeperiod=14)
        df['SAR'] = ta.SAR(df['High'], df['Low'], acceleration=0, maximum=0)
        df['Boling_upper'], df['Boling__middle'], df['Boling_lower'] = ta.BBANDS(df['Close'], timeperiod=10, nbdevup=2, nbdevdn=2)
...
##        if df['CCI'][-1] > 100 and round(df['Close'][-1]-df['Close'].shift(1)[-1],2) < 0 and round(df['Close'][-1]-df['Close'].shift(2)[-1],2) < 0:
####            print(m,c1x,'  ','\n') 
##            print(i,' of ',len(all),x,' == SAR Buy ==','cci=',df['CCI'][-1],'   sell ATR',df['ATR'][-1],'  ',df['Close'][-1],
##                  ' [price_movt ---> ',
##                  round(df['Close'][-1]-df['Close'].shift(1)[-1],2),'/',
##                  round(df['Close'][-1]-df['Close'].shift(2)[-1],2),'/',
##                  round(df['Close'][-1]-df['Close'].shift(3)[-1],2),'/',
##                  round(df['Close'][-1]-df['Close'].shift(5)[-1],2),'][',
##                  c1x,'  ',round(m),'==',c1xb,'=='
##                  )
##            pp_sell.append(x)
##        elif df['CCI'][-1] < -100 and round(df['Close'][-1]-df['Close'].shift(1)[-1],2) > 0 and round(df['Close'][-1]-df['Close'].shift(2)[-1],2) > 0:
####            print(m,c1x,'  ','\n')
##            print(i,' of ',len(all),x,' ==SAR Buy==','cci=',df['CCI'][-1],'   sell ATR',df['ATR'][-1],'  ',df['Close'][-1],
##                  'price_movt_from_prev2 ---> ',
##                  round(df['Close'][-1]-df['Close'].shift(1)[-1],2),'/',
##                  round(df['Close'][-1]-df['Close'].shift(2)[-1],2),'/',
##                  round(df['Close'][-1]-df['Close'].shift(3)[-1],2),'/',
##                  round(df['Close'][-1]-df['Close'].shift(5)[-1],2),
