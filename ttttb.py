...
##        df['Volumea']=(df['Volume']-df['Volume'].shift(1))
    ...
    tt=pd.concat([df['direct'],df['HA'],df['Close'],df['Close_ch'],df['Volume'],
                  ...
                  df['EMA_5s'],df['EMA_10s'],df['EMA_21s'],df['EMA_50s'],df['EMA_100s'],df['EMA_200s'],df['ticker']],axis=1)

##    print(tt.tail(4))
    print('tt-columns:',tt.columns)
    
##    sys.exit()
    tt.columns=['direct', 'HA', 'Close', 'Close_ch', 'Volume', 'Volumea', 'CCI', 'DX',
       ...
       'EMA_21s', 'EMA_50s', 'EMA_100s', 'EMA_200s','ticker' 
                ]
    ...
    tt.set_index('Date')
    print('\n',tt.columns,'\n')
    print(tt.columns.get_loc('ticker'))

    gg33=tt.iloc[:,[57,0,1,2,3,4,7,10]].tail(135)
    print(gg33)
