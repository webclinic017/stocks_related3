    ...
    gg33=tt[['index', 'Datetime','ticker', 'pivot','pivotx','pp','*','Close_1d_ch','*','r1','r2','r3','*','s1','s2','s3','*','volatility','*',
            ...
            'ppx','*','r1x','r2x','r3x','*','s1x','s2x','s3x','*','*',
             ...
             'Price_from_BBUP','Price_from_BBLWR','Price_from_BBMid','bol_wd',
              ...
              ,'upward_pressure','downward_pressure','Closev','5_10_crossovr','signal4'
...
####################### 5-10 crossover ###########################3
    pp44=[]
##    gg33b['5_10_crossovr'] = gg33b['5_10_crossovr']. astype(float)  | (gg33b['5_10_crossovr'].shift(1).loc[x].item() > 0)
##    for x in range(gg33b.shape[0]):
    for x in gg33b.index:    
##    for x in range(gg33b.shape[0]):    
        if gg33b['5_10_crossovr'].loc[x][0] > 0:
            
##            print('jjjjjjjjjjjjjjjj   == ', gg33b['5_10_crossovr'].loc[x])
##        print(type(float(float(gg33b['5_10_crossovr'].loc[x]))))
##        if float(gg33b['5_10_crossovr'].loc[x]) < 0 and float(gg33b['5_10_crossovr'].shift(1).loc[x]) > 0:
##            pp44.append(gg33b['Datetime'].loc[x])
            gg33b['signal4'].loc[x]='ii'
            print('sssssssssssssssss    ',gg33b['signal4'].loc[x])
##            gg33b['signal4'].loc[x]=='xx'
    pp45=[]
##    for x in range(gg33b.shape[0]):
####        print(gg33b['5_10_crossovr'].loc[x])
##        if gg33b['5_10_crossovr'].loc[x].any() > 0 and gg33b['5_10_crossovr'].shift(1).loc[x].any() < 0:
##            pp45.append(gg33b['Datetime'].loc[x])
##            gg33b['signal4'].loc[x]=='yy'
##################################################################
####################### 10-21 crossover ###########################3
    pp46=[]
    for x in range(gg33b.shape[0]):
##        print(gg33b['10_21_crossovr'].loc[x])
        if gg33b['10_21_crossovr'].loc[x] < 0 and gg33b['10_21_crossovr'].shift(1).loc[x] > 0:
            pp46.append(gg33b['Datetime'].loc[x])
    pp47=[]
    for x in range(gg33b.shape[0]):
##        print(gg33b['5_10_crossovr'].loc[x])
        if gg33b['10_21_crossovr'].loc[x] > 0 and gg33b['10_21_crossovr'].shift(1).loc[x] < 0:
            pp47.append(gg33b['Datetime'].loc[x])
##################################################################    
####################### '21_50_crossovr' ###########################3
    pp48=[]
    for x in range(gg33b.shape[0]):
...
################################################################## 
    ...
    print('stupid azhar  ',gg33b,'\n')

    print('\n\n','**********************************')
    print('i am stupid'),len(gg33b['Datetime'])
    print('\n\n','**********************************')
          
            
    gg33b.set_index('Datetime',inplace=True)
...
# clayton
        plt.text(xc,0,'50_100x-up',rotation=90)
        print(xc)
################################################################# 
...
##    plt.xticks(df['Datetime'],values)
##    plt.xticks(np.arange(min(df['Datetime']), max(df['Datetime'])+1, 1.0))
