...
##           df['HA'].loc[x] > 5:
    ...
    for x in range(gg[-1],df.shape[0]):
# sabir test
##        if df['Close'].loc[x] > 420 and df['Close'].loc[x] < 421:
        if df['Close'].loc[x] > 430 and df['Close'].loc[x] < 433:
            k3=x
            gg.append(x)
...
##    for x in range(ss[-1],df.shape[0]):
##    for x in range(gg[-1],df.shape[0]):    
    for x in df.index:  
        df['Low'].loc[x]=df['Low'].loc[x]
        df['High'].loc[x]=df['High'].loc[x]
...
##                      k3s,'****   buy= ',m33.round(2), ' buy row no k3=',k3)
                ...
                print(x,' 33(sell signal--> )',df['Datetime'].loc[x],'  ',m33.round(2),'/',(m33n).round(2),'  ','[profit]=',(m33n-m33).round(2), '  in ',
