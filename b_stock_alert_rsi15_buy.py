...
'''
3 defs i.e.
def s5: reads data from entire stock market tickers, narrow down based on adx and atr.
def bb: add more calculation.
def cc: proccess
'''
import inspect
import sys
...
###############################################################################
        ...
        df['LINEARREG_INTERCEPT'] = ta.LINEARREG_INTERCEPT(
    ...
    # df['ADX_over25_strong_less25_weak_trend']=''
        ...
        df['aroondown'], df['aroonup'] = ta.AROON(
            np.asarray(
                df['High']), np.asarray(
                df['Low']), timeperiod=14)
    ...
    # original        df['SAR'] = ta.SAR(df['High'], df['Low'],
    # acceleration=90, maximum=5)
        df['SAR'] = ta.SAR(df['High'], df['Low'], acceleration=300, maximum=5)
        df['Boling_upper'], df['Boling__middle'], df['Boling_lower'] = ta.BBANDS(
            df['High'], timeperiod=20, nbdevup=2, nbdevdn=2)
        df['Boling_upper2'], df['Boling__middle2'], df['Boling_lower2'] = ta.BBANDS(
            df['Low'], timeperiod=20, nbdevup=2, nbdevdn=2)
        df['SARx'] = df['Close'] - df['SAR']
        df['ADX_over25_strong_less25_weak_trend'] = ta.ADX(
            df['High'], df['Low'], df['Close'], timeperiod=3).round(2)
        df['ticker'] = x
    ...
    # def in_squeeze(df):
    # return df['lower_band'] > df['lower_keltner'] and df['upper_band'] <
    # df['upper_keltner']
...
# print(df,'kk55',x)
        ...
        df['Low_delta'] = df['Low'] - df['Low'].shift(1)
        df['High_delta'] = df['High'] - df['High'].shift(1)
        df['Red_breath'] = df['Open'] - df['High']
        df['Open_delta'] = df['Open'] - df['Open'].shift(1)
...
# print(df.index,'kkkkkkkkkkkkkkkkkkkkkkkkkkk')
    for z in df.index:
        ...
        buy_condition = df['MOM'].loc[z] - df['MOM'].shift(
            1).loc[z] > 0 and df['macd'].loc[z] > 0 and df['adx'].loc[z] > 24 and df['ATR'].loc[z] > .9 and df['TR'].loc[z] > .5
...
######################## SELL CONDITION ##################################
            ...
            for z in range(g43, len(df.index)):
                sell_condition = (
                    df[mm3].loc[z] < 0 and (
                        df[mm3].loc[z] -
                        df[mm3].shift(1).loc[z] < 00) and df['Close'].loc[z] > v5)
...
'''
        df=df[['Datetime','ticker','signal','Close','MOM','macd','adx','ATR','TR','Close_vwap',\
               'macd',
               'macdsignal',
               'macdhist',
                'x',
