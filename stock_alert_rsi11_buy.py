...
##            df = df['High'].dropna(axis=0, how='all')
    ...
    ##                df['cross'].loc[z]='down'+str(df['EMA-35'].loc[z])
            ...
            if p > 0:
                print(x,' Squeeze Alert','  ', inspect.getframeinfo(inspect.currentframe()).lineno)
...
##            df3=df
    ...
    ##                 df['EMA_50'].loc[x] > df['vwap'].loc[x]
                ...
                uu3=df['MOM'].loc[x]-df['MOM'].shift(1).loc[x] > 0 and\
                     df['macd'].loc[x]>0 and df['adx'].loc[x]>24 and df['ATR'].loc[x]>.9 and df['TR'].loc[x]>.5
...
##                        print('\n\n')
                        ...
                        print('***',df.iloc[-1,1],df,'\n','   ',' df sell signal condition met @',v4,'  line no:', inspect.getframeinfo(inspect.currentframe()).lineno)
...
##                        print('\n')
                        ...
                        profit=(v4-v5)
                        if profit < 0:
                            print('\n',df.iloc[-1,1],'-------Last tranacation was a loss-------------',profit,'  ', inspect.getframeinfo(inspect.currentframe()).lineno)
                        elif profit > 0:
                            print('\n',df.iloc[-1,1],'---------Last tranacation was a Profit-------------',profit,'  ', inspect.getframeinfo(inspect.currentframe()).lineno)
...
##                        print(df,'\n',df.iloc[-1,1],'sell sell signal condition met','  ', inspect.getframeinfo(inspect.currentframe()).lineno)
    ...
    ##            b.sort_values(by=[3])
            if b.shape[0]>0:
                ...
                print(x5,'kjkkkkk','  ', inspect.getframeinfo(inspect.currentframe()).lineno)
...
s4()
print(__file__,'  ', inspect.getframeinfo(inspect.currentframe()).lineno)
##print(gg5)
print('\n\n\n')
print('***************************************************************************')
