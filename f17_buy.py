def inputs(df,buy_price_gg,x,xs,gft,gft2):
    import f16_condt
    from f16_condt import con_buy_down
...
##    print('hhhh',df)
    ...
    for x in df.index:
        dfq=df.iloc[x,:]
        buy_condition=f16_condt.con_buy_down(dfq,x,xs)
##        buy_condition=(df['macd'].loc[x] < 0)
        
##        buy_condition=con_buy_down(df,x,xs)
...
def f17_buyp(df,buy_price_gg,xs,gft,gft2):    
##    print('hello - in f17_buy')        
...
##        print('buy price from f17_buy= ',buy_price_gg)
