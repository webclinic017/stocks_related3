...
##ticker='^dji'
ticker='^ndx'
df = yf.download(ticker, period=perda, interval=intervla,prepost = True)
df=pd.DataFrame(df)
...
print('Ticker=',ticker,'     interval=',intervla,'     current_Price=',df4['Close'].loc[df4.shape[0]-1])
d2=round(b/(b+c)*100,2)  #green
d3=round(c/(b+c)*100,2)  # red
...
##print('\n\n')
##print('\n\n',Green_V_cts,'    ',len(Green_V_cts),' peter33  ',pp,)
##print('\n\n')
##for x in Green_V_cts[len(Green_V_cts)-int(pp):]:
for x in Green_V_cts[-int(pp):]:
##for x in Green_V_cts[len(Green_V_cts)-int(pp):]:
    y +=x
##    print(x,'Green')
##print('sum of Greens/Vol in last 5 mins:',y)
for xp in Red_V_cts[-int(pp):]:
...
print('Sell-Last', int(pp),'_records_Sum_of_total_Red_Vol=', numerize.numerize(yp),'/',d3,' %')
