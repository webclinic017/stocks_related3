...
print('Buy-Last', int(pp),'_records_Sum_of_total_Green_Vol=',numerize.numerize(y),'/',round(y/(y+yp)*100,2),' %')
print('Sell-Last', int(pp),'_records_Sum_of_total_Red_Vol=', numerize.numerize(yp),'/',round(yp/(y+yp)*100,2),' %')
print('Last ',int(pp),' records,delta_Vol=',numerize.numerize(y-yp))
...
df4=df4[['Date_','Hour_','Dayp','Ticker_','Close','Green_Buy_Vol','Red_Sell_Vol2','Delta_Vol','TBD']]
##print(df4)
##print('\n\n')
##sys.exit()
...
for x in df4.index:
    df4['Green_Buy_Vol'].loc[x]=numerize.numerize(np.int64(df4['Green_Buy_Vol'].loc[x]).item())
    if df4['Red_Sell_Vol2'].loc[x] !='':
        df4['Red_Sell_Vol2'].loc[x]=numerize.numerize(np.int64(df4['Red_Sell_Vol2'].loc[x]).item())
    if df4['Delta_Vol'].loc[x] !='':
        df4['Delta_Vol'].loc[x]=numerize.numerize(np.int64(df4['Delta_Vol'].loc[x]).item())    
